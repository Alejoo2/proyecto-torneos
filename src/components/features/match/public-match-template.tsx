...
import { Inbox, Lock, SearchX, Users } from "lucide-react";
...
export function PublicMatchTemplate({ matchId }: { matchId: string }) {
  ...
  // Espejo del engine: EDITABLE_BLOCKERS
  const editable = match ? !(TERMINAL_MATCH_STATUS as readonly string[]).includes(match.status) : false;
  const captainTeamId =
    viewer && match
      ? viewer.captainOfTeamIds.find((id) => id === match.homeTeamId || id === match.awayTeamId) ?? null
      : null;
  ...
  return (
    <div className="pb-nav-safe pt-14">
      ...
      <div className="mt-4 space-y-4 px-5">
        ...
        {/* Vista Capitán: solo capitán VIGENTE de uno de los dos equipos, y el partido
            no debe estar terminal (espejo del engine). Server-derived vía `viewer`. */}
        {captainTeamId && editable && myTeam && (
          <CaptainAbsenteePanel
            ...
            onToggle={(c, isAbsent) =>
              markAbsentMutation.mutate({ matchId: match.id, playerId: c.playerId, isAbsent, notes: c.notes ?? undefined })
            }
            onNotesCommit={(c, notes) =>
              markAbsentMutation.mutate({ matchId: match.id, playerId: c.playerId, isAbsent: c.isAbsent, notes })
            }
          />
        )}
